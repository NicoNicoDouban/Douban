    ...
    def getHotText(request):
        ...
        if type=="articles":
            for article in result:
                temp = {'id':article.id,'title': article.title, 'author': article.author.username, 'pub_time': str(article.pub_time),
...
class userInfo(View):
    ...
    def getInfo(request):
        ...
        if users.judgeExist(type="id",text=id):
            try:
                user=Users.objects.get(id=id)
                info={'birthday':user.birthday,'gender':user.gender,'address':user.address,'username':user.username,'email':user.email}
                context.append(info)
                resp = {'rsNum': 1}
                context.append(resp)
                return HttpResponse(json.dumps(context), content_type="application/json")
    ...
    def getComments(request):
        ...
        if type=="book":
            ...
            for comment in results:
                temp={"id":comment.id,"pub_time":comment.pub_time,'text':comment.text,'commenterId':comment.commenter_id,'commenterName':comment.commenter_id.username}
                context.append(temp)
        elif type=="user":
            results=models.Comments.objects.filter(commenter_id=id)
            for comment in results:
                temp={"id":comment.id,"pug_time":comment.pub_time,'text':comment.text,'bookId':comment.book_id,'bookName':comment.book_id.name}
                context.append(temp)
        return HttpResponse(json.dumps(context), content_type="application/json")
